	...
	return (
		<div className="sticky top-0 left-0 right-0 z-50 h-fit p-2 border-b flex items-center px-4 bg-white">
			<div className="flex justify-start gap-4">
				<button
					className="text-black rounded-lg transition-colors duration-200 disabled:text-gray-400 disabled:cursor-not-allowed"
					disabled={true}
				>
					<Undo2 />
				</button>
			...
			<div className="flex gap-0.5 justify-center flex-1">
				<div
					className={`flex flex-col items-center gap-0.5 p-1.5 rounded-lg cursor-pointer transition-colors duration-200 h-12 ${
						pdf && pdf.mode == "text"
				...
				<div
					className={`flex flex-col items-center gap-0.5 p-1.5 rounded-lg cursor-pointer transition-colors duration-200 w-14 h-12 ${
						pdf && pdf.mode == "draw"
							? "bg-blue-100 text-blue-600"
							: "hover:bg-blue-50 hover:text-blue-500 text-gray-600"
					}`}
				...
				<div
					className={`flex flex-col items-center gap-0.5 p-1.5 rounded-lg cursor-pointer transition-colors duration-200 w-12 h-12 ${
						pdf && pdf.mode == "sign"
							? "bg-blue-100 text-blue-600"
							: "hover:bg-blue-50 hover:text-blue-500 text-gray-600"
					}`}
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								annotations you added in all pages.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
							<AlertDialogCancel>Cancel</AlertDialogCancel>
							<AlertDialogAction onClick={clearAnnotations}>
								Continue
							</AlertDialogAction>
						</AlertDialogFooter>
					</AlertDialogContent>
			...
			<div className="flex justify-end gap-4">
				<button
					className="flex items-center justify-center rounded-lg bg-white cursor-pointer transition-colors duration-200 border border-transparent disabled:cursor-not-allowed disabled:text-gray-400"
					disabled={true}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
				...
				<button
					className="flex items-center justify-center cursor-pointer rounded-lg duration-200"
					aria-label="Download"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
