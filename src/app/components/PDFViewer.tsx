...
export default function PDFViewer({ url }: {url: string}) {
  ...
  const [annotationToolPosition, setAnnotationToolPosition] = useState<Position>({x: 0, y: 0})
  const [showAnnotationTool, setShowAnnotationTool] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const text = currentAnnotations.textAnnotations.find(annotation => annotation.id === selectedAnnotationId);
    ...
    if (image) {
      setAnnotationToolPosition({x: image.x, y: image.y})
    }

  }, [selectedAnnotationId, currentPage, pageAnnotations])
