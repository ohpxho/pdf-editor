     ...
     <div className="w-48 relative h-full bg-white border-r border-gray-200 shadow-sm overflow-y-scroll pb-4">
        <div className="p-4 border-b w-full border-gray-200 flex items-center justify-between">
            <div className="text-sm font-bold text-gray-700">
                <span>{pdf? pdf.currPageInView+1: 0} of {numPages} <span className="font-normal">{numPages === 1 ? 'page' : 'pages'}</span></span>
            </div>
            <button 
            ...
            <Document className="relative flex flex-col gap-4 bg-transparent w-full" file={pdf?.url} onLoadSuccess={onDocumentLoadSuccess} options={options}>
              {Array.from(new Array(numPages), (_el, index) => {
                ...
                >
                  ...
                  />
                  <div className={`relative text-xs ${pdf && pdf.currPageInView == index? 'text-blue-600': 'text-gray-500'} font-medium mt-1`}>
                    Page {index + 1}
                  </div>
                </div>
              )})}
